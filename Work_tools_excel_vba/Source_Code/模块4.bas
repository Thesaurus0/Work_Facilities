...
End Sub
Sub t()
    Dim a As LongPtr, za As LongPtr, zb As LongPtr
    a = 100
    za = VarPtr(a)    'za???????,?????a???100??????????
    zb = VarPtr(100)    '??,100???????????,????????,??????
    Debug.Print za, zb '??????????
End Sub
...
Sub jh()
    ...
    Debug.Print b
    a = 3000
    CopyMemory b, ByVal VarPtr(a), LenB(a)
    Debug.Print b
    CopyMemory ByVal VarPtr(b), 4000&, 4
    Debug.Print b
    a = 5000
    CopyMemory ByVal VarPtr(b), a, LenB(a)
    Debug.Print b
    a = 6000
    CopyMemory ByVal VarPtr(b), ByVal VarPtr(a), LenB(a)
    Debug.Print b
End Sub
